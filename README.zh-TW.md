...
Nautilus 對 Unix Domain Sockets (UDS) 採用嚴格的權限模型，在確保安全與服務隔離的同時，維持高性能通訊。

### 目錄結構與權限

| 目錄 | 權限 | 用途 |
| :--- | :--- | :--- |
| `/var/run/nautilus/services` | `1777` (Sticky Bit) | 後端服務放置 `.sock` 檔案的地方。 |
| `/var/run/nautilus/entrypoints` | `0755` | Nautilus 建立入口 Socket 的地方。 |

### 安全模型

1.  **隔離 (Sticky Bit)**：`services` 目錄設定了 **Sticky Bit** (`1777`)。這允許任何後端服務建立自己的 Socket，但防止服務刪除或重命名他人擁有的 Socket。
2.  **存取控制 (ACL)**：在生產環境 (Docker) 中，Nautilus 使用 **POSIX ACLs** (存取控制列表) 自動授予 `nautilus` 使用者對 `services` 目錄中建立的任何 Socket 的讀寫權限，即使 Socket 的擁有者限制了其他使用者的存取。
3.  **權限降級 (Privilege Dropping)**：Nautilus Docker 鏡像以 `root` 身分啟動以初始化這些權限，然後立即降權至非 root 的 `nautilus` 使用者執行。

### 後端實作建議

為了維持安全的環境，後端服務應該：
-   **建議權限**：將您的 Socket 權限設定為 `0700`（僅擁有者）或 `0770`（擁有者與群組）。
-   **依賴 ACL**：**不要**使用 `0666`。`services` 目錄上的「預設 ACL」會自動授予 `nautilus` 使用者所需的權限。
