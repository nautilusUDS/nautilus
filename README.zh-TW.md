...
| 目錄 | 用途 |
| :--- | :--- |
| `/var/run/nautilus/services` | 後端服務放置 `.sock` 檔案的地方。 |
| `/var/run/nautilus/entrypoints` | Nautilus 建立入口 Socket 的地方。 |

### 安全模型

1.  **權限降級 (Privilege Dropping)**：Nautilus Docker 鏡像以 `root` 身分啟動以初始化環境權限，然後立即降權至非 root 的 `nautilus` 使用者執行。
2.  **自動化環境管理**：Nautilus 會自動配置目錄的隔離與存取控制，以確保服務間的通訊安全性。
